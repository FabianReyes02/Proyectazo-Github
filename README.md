# 🎮 Proyectazo-Github

Proyecto semestral de la asignatura de GitHub.  
Se trata de una **página web de catálogos de videojuegos**, desarrollada con tecnologías web como HTML, CSS, JavaScript y PHP.

![EstimInfos](https://github.com/user-attachments/assets/781dcef4-00bf-4c74-bafd-a4353281f476)

---

## 📑 Tabla de contenido

- [📝 Descripción del Proyecto](#descripción-del-proyecto)
- [👨‍💻 Integrantes](#integrantes)
- [🛠️ Tecnologías](#tecnologías)
- [💻 Instalación](#instalación)
- [📅 Avances Semanales](#avances-semanales)
- [🚀 Despliegue](#despliegue)
- [🪪 Licencia](#licencia)

---

## 📝 Descripción del Proyecto

Página web con un catálogo de videojuegos que muestra imágenes, descripciones y detalles, con vistas organizadas para el usuario.  
En desarrollo se incluirá un sistema de autenticación y conexión con bases de datos.

---

## 👨‍💻 Integrantes 

| Nombre            | Rol                  |
|-------------------|----------------------|
| Matías Vargas     | Desarrollador Backend |
| Martín Rivas      | Desarrollador Frontend |
| Fabían Reyes      | Líder del Proyecto    |
| Thomas Alvarez    | Desarrollador Backend |

---

## 🛠️ Tecnologías

Aquí están las tecnologías usadas en este proyecto:

| Tecnología | Logo |
|------------|------|
| HTML       | <img src="https://github.com/user-attachments/assets/c2c1fe59-1ec4-4858-826c-df552c203aac" width="30" /> |
| CSS        | <img src="https://github.com/user-attachments/assets/512c2801-1e0a-442e-8182-48c682113929" width="30" /> |
| JavaScript | <img src="https://github.com/user-attachments/assets/49d62683-7df1-4e03-a524-fc0c4031a169" width="30" /> |
| PHP        | <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg" width="30"/> |
| Git        | <img src="https://github.com/user-attachments/assets/91e9a0ac-9178-4021-9099-7244e6504f27" width="30" /> |
| GitHub     | <img src="https://github.com/user-attachments/assets/b4082a0f-115b-47df-8113-ce780ebf1f7a" width="30" /> |

---

## 💻 Instalación

1. Clonar el repositorio:

```bash
git clone https://github.com/FabianReyes02/Proyectazo-Github.git
````
2.- Ingresar a directorios
```bash
cd Proyectazo-Github
cd Proyecto
cd home
```
3.- Ejecutar archivo
```bash
"home.html"
```
---
4.-
```bash
Instalar la extensión Allow Cords para correcto funcionamiento
```
---

## Avances semanales

|Semanas            |Avances               |Link |
|-------------------|----------------------|-----|
|Semana 1           |Inicio del proyecto   |[Ver]([Avances/semana1/Proyeto Github](https://github.com/FabianReyes02/Proyectazo-Github/tree/main/Avances/semana1/Proyeto%20Github)
|Semana 2           |Integración de imagenes y breves descripcion de los juegos|[Ver](https://github.com/FabianReyes02/Proyectazo-Github/tree/main/Avances/semana2/Proyectazo-Github-main)
|Semana 3           |Avance del home       |[Ver](https://github.com/FabianReyes02/Proyectazo-Github/tree/main/Avances/semana3)
|Semana 4           |Código base html      |[Ver]()
|Semana 5           |Avance de diseño      |[Ver](https://github.com/FabianReyes02/Proyectazo-Github/tree/main/Avances/semana5)
|Semana 6           |Tabla gantt           |[Ver](https://github.com/FabianReyes02/Proyectazo-Github/tree/main/Avances/semana%206)
|Semana 7           |Avance de diseño      |[Ver](https://github.com/FabianReyes02/Proyectazo-Github/tree/main/Avances/semana7)
|Semana 8           |Se agregan juegos     |[Ver](https://github.com/FabianReyes02/Proyectazo-Github/tree/main/Avances/semana8)
|Semana 9           |Evaluación            |[Ver]()
|Semana 10          |Cambio Frontend       |[Ver]()

## 🪪 Licencia
Este proyecto está bajo la Licencia MIT. Consulta el archivo LICENSE para más detalles.


